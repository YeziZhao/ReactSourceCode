...
## 2.8.6 子元素挂载
上面的挂载中，整体都是挂在一个ComponentCompositeComponent。里面的子元素的挂在详细我们接下来再进行分析。当我们的元素是ReactDOMComponent类型的时候，在ReactReconclier.mountComponent内部，调用internalInstance.mountComponent(xx,xx),其实是调用的ReactDOMComponent模块的方法。
该方法内部如下：
```javascript
ReactDOMElement = {
