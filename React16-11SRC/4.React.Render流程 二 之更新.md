...
```
## 6. 进入Schedule流程
### 6.1 scheduleUpdateOnFiber
https://juejin.im/post/6844903943017857031
不管是首次render还是update节点，都会进入该方法。
```javascript
function scheduleUpdateOnFiber(fiber: Fiber,lane: Lane, eventTime: number,) {
  ...
  // TODO: requestUpdateLanePriority也读取优先级。 将优先级作为参数传递给该函数和该函数。
  // 获取当前render的优先级?该函数是scheduler中的全局变量
