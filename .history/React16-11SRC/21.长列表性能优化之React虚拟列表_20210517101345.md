...
### 滚动触发注册事件
将onScroll通过`[componentDidMount]`注册到dom上。滚动事件中，使用的`requestAnimationFrame`,该方法是利用浏览器的空余时间进行执行，可以提高代码的性能。大家想进行深入理解，可以去查阅该api的具体使用。下面我们分析依稀重点：
- 每次滚动，触发数据变更，优先查询，滚动位置的 startIndex
